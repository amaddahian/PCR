...
status_roach_clusters() {
  ...
  # Version checks (both sides)
  _status_sql_or_prompt CA "SELECT VERSION();" system root
  _status_sql_or_prompt CB "SELECT VERSION();" system root
...
# ===========================
# In-place Rolling Upgrades (Patch & Major) + Finalize + Rollback
# ===========================

# Parse a CockroachDB version string like 'v24.1.7' or '24.1.7' → MAJ MIN PATCH
_parse_semver() {
  local v="${1:?}"
...
_role_ports() {
  local role="${1:?CA|CB}"
  case "$role" in
    CA|ca|a|1|primary|p) echo "26357 8080 26257" ;;  # inter http sql (container base ports)
    CB|cb|b|2|standby|s) echo "27357 8090 27257" ;;  # inter http sql
    *) return 1 ;;
  esac
}

_get_host_ports() {
  # Echo "host_http host_sql" or blank if unknown
  local runtime="${1:?}" name="${2:?}"
  local host_http="" host_sql=""
  while IFS= read -r line; do
    cport="${line%%/*}"; hport="${line##*:}"
    [[ "$cport" =~ ^[0-9]+$ && "$hport" =~ ^[0-9]+$ ]] || continue
    (( cport >= 8000 && cport < 9000 ))  && host_http="$hport"
    (( cport >= 26000 && cport < 28000 )) && host_sql="$hport"
  done <<< "$($runtime port "$name" 2>/dev/null || true)"
  echo "${host_http:-} ${host_sql:-}"
}

_wait_node_ready_sql() {
  # Poll SELECT 1 on system tenant until OK for the given role/node ordinal
  local role="${1:?}" ordinal="${2:?}" runtime="${3:-$(_default_runtime)}"
  local i rc=1
  for ((i=1; i<=READY_MAX_ITERS; i++)); do
    set +e
    run_roach_sql "$role" "SELECT 1;" system root "$ordinal" "$runtime"
    rc=$?
    set -e
    if (( rc == 0 )); then
      _printc "$GREEN_BOLD" "[node ${ordinal}] SQL reachable."
      return 0
    fi
    _printc "$YELLOW_BOLD" "[node ${ordinal}] Waiting for SQL... (${i}/${READY_MAX_ITERS})"
    sleep "$READY_POLL_INTERVAL"
  done
  _printc "$RED_BOLD" "[node ${ordinal}] Timed out waiting for SQL."
  return 1
}

_get_join_csv() {
  # Build join csv like "roach4:27357,roach5:27357,roach6:27357" without touching STDIN and without calling nested helpers that could block.
  local runtime="${1:?}" role="${2:?}"
  local inter http sql; read -r inter http sql < <(_role_ports "$role")
  # Direct discovery: list all roach* on roachnet1, filter by role based on container SQL port (26257 => CA, 27257 => CB)
  local all; all="$($runtime ps -a --filter "network=roachnet1" --format '{{.Names}}' | grep -E '^roach[0-9]+$' || true)"
  local keep=()
  for n in $all; do
    # Identify side by checking which CONTAINER port is exposed
    local ports; ports="$($runtime port "$n" 2>/dev/null || true)"
    if [[ "$role" =~ ^(CA|ca|a|1|primary|p)$ ]]; then
      grep -q '^26257/tcp' <<< "$ports" && keep+=("$n")
    else
      grep -q '^27257/tcp' <<< "$ports" && keep+=("$n")
    fi
  done
  # Sort numerically by roach index to keep order
  local sorted; sorted="$(printf "%s\n" "${keep[@]}" | sed 's/^roach//' | sort -n | sed 's/^/roach/')" 
  local csv=""
  for n in $sorted; do
    [[ -z "$n" ]] && continue
    csv+="${csv:+,}${n}:${inter}"
...
_run_with_image_same_ports() {
  # (re)Start a node with same name/vol/ports but different image
  # args: runtime name image role ordinal
  local runtime="${1:?}" name="${2:?}" image="${3:?}" role="${4:?}" ordinal="${5:?}"
  local inter http_base sql_base; read -r inter http_base sql_base < <(_role_ports "$role")
  local host_http host_sql; read -r host_http host_sql < <(_get_host_ports "$runtime" "$name")
  local http_c=$(( http_base + ordinal - 1 ))
  local sql_c=$(( sql_base + ordinal - 1 ))
  local http_h="${host_http:-$http_c}"
  local sql_h="${host_sql:-$sql_c}"
  local vol="roachvol${name#roach}"
  local network="roachnet1"
  local join_csv; join_csv="$(_get_join_csv "$runtime" "$role")"

  _printc "$CYAN_BOLD" "Starting ${name} with image=${image} (HTTP ${http_h}:${http_c}, SQL ${sql_h}:${sql_c}) ..."
  _exec "$runtime" run -d \
    --name="${name}" --hostname="${name}" --net="$network" \
    -p "${sql_h}:${sql_c}" -p "${http_h}:${http_c}" \
    -v "${vol}:/cockroach/cockroach-data" \
    "$image" start \
      --advertise-addr="${name}:${inter}" \
      --http-addr="${name}:${http_c}" \
      --listen-addr="${name}:${inter}" \
      --sql-addr="${name}:${sql_c}" \
      --insecure \
      --join="$join_csv"
}
...
_replace_node_image() {
  local runtime="${1:?}" name="${2:?}" image="${3:?}"
  _printc "$PURPLE_BOLD" "Replacing image on ${name} → ${image}"
  _exec "$runtime" stop "$name" >/dev/null 2>&1 || true
  _exec "$runtime" rm -f "$name" >/dev/null 2>&1 || true
  # NOTE: caller restarts with proper args via _run_with_image_same_ports
}

_rolling_replace_image_for_role() {
  # args: role target_version [runtime]
  local role_in="${1:?}" tgt_ver_in="${2:?}" runtime="${3:-$(_default_runtime)}"
  local role; role="$(_canon_cluster "$role_in" 2>/dev/null || echo "$role_in")"
  local resolved; resolved="$(_resolve_and_pull_image "$runtime" "$tgt_ver_in" "$CYAN_BOLD")"
  local rt image; rt="$(echo "$resolved" | sed -n '1p')" ; image="$(echo "$resolved" | sed -n '2p')"
  runtime="$rt"

  local names; names="$(_get_names_by_role "$runtime" "$role")"
  if [[ -z "$names" ]]; then
    _printc "$RED_BOLD" "No nodes found for role=${role}"
    return 1
  fi
  ...
  for name in $names; do
    [[ -z "$name" ]] && continue
    ordinal=$(( ordinal + 1 ))
    _replace_node_image "$runtime" "$name" "$image"
    _run_with_image_same_ports "$runtime" "$name" "$image" "$role" "$ordinal"
    _wait_node_ready_sql "$role" "$ordinal" "$runtime" || {
      _printc "$RED_BOLD" "Node ${name} failed to become ready; aborting rolling change."
      return 1
    }
  done

  _printc "$GREEN_BOLD" "✓ Completed rolling image change on ${role}."
}

_current_cluster_version() {
  # Echo active cluster version like v24.1 (no patch) if available
  local role_in="${1:?}" runtime="${2:-$(_default_runtime)}"
  local role; role="$(_canon_cluster "$role_in" 2>/dev/null || echo "$role_in")"
  local node csql; read -r node csql < <(_role_node_and_sqlport "$runtime" "$role") || { echo ""; return 1; }
  local url="postgresql://root@${node}:${csql}?options=-ccluster=system&sslmode=disable"
  local out
  out="$($runtime exec -i "$node" ./cockroach sql --format=tsv --insecure --url "$url" --execute "SHOW CLUSTER SETTING version;" 2>/dev/null || true)"
  echo "$out" | awk 'NR==2{print $1}'
}

_finalize_major_upgrade_role() {
  local role="${1:?}" runtime="${2:-$(_default_runtime)}"
  _printc "$CYAN_BOLD" "Finalizing any pending major-version upgrade on ${role}..."
  set +e
  run_roach_sql "$role" "RESET CLUSTER SETTING cluster.preserve_downgrade_option;" system root 1 "$runtime" || true
  run_roach_sql "$role" "SHOW CLUSTER SETTING cluster.preserve_downgrade_option;" system root 1 "$runtime" || true
  run_roach_sql "$role" "SHOW CLUSTER SETTING version;" system root 1 "$runtime" || true
  set -e
  _printc "$GREEN_BOLD" "Requested finalization on ${role}."
}

# ----- Interactive wrappers -----

upgrade_patch_interactive() {
  heading "Rolling PATCH change" "Upgrade/Downgrade within same major.minor"
  local runtime="${1:-$(_default_runtime)}"
  echo -n "Which side? [A=CA, B=CB] (default A): "; read -r side; side="${side:-A}"
  local role; role="$(_canon_cluster "$side" 2>/dev/null || echo "$side")"

  local cur; cur="$(_current_cluster_version "$role" "$runtime")"
  [[ -n "$cur" ]] && _printc "$CYAN_BOLD" "Current cluster version on ${role}: ${cur}" || _printc "$YELLOW_BOLD" "(Could not detect current version; continuing.)"
  echo -n "Target PATCH version (e.g., 24.1.18): "; read -r tgt
  if [[ -z "$tgt" ]]; then echo "No version entered; aborting."; return 0; fi

  if [[ -n "$cur" ]]; then
    local CM CN CP TM TN TP; read -r CM CN CP < <(_parse_semver "$cur"); read -r TM TN TP < <(_parse_semver "$tgt")
    if [[ "$CM" != "$TM" || "$CN" != "$TN" ]]; then
      _printc "$RED_BOLD" "Refusing: patch changes must stay within the same MAJOR.MINOR (${CM}.${CN})."
      return 1
    fi
  fi
  _confirm "Proceed with rolling patch change on ${role} → ${tgt}? [y/N]: " || return 0
  _rolling_replace_image_for_role "$role" "$tgt" "$runtime"
}

upgrade_major_interactive() {
  heading "Rolling MAJOR upgrade" "One node at a time, then finalize"
  local runtime="${1:-$(_default_runtime)}"
  echo -n "Which side? [A=CA, B=CB] (default A): "; read -r side; side="${side:-A}"
  local role; role="$(_canon_cluster "$side" 2>/dev/null || echo "$side")"

  local cur; cur="$(_current_cluster_version "$role" "$runtime")"
  [[ -n "$cur" ]] && _printc "$CYAN_BOLD" "Current cluster version on ${role}: ${cur}"
  echo -n "Target MAJOR version (e.g., 24.2.0 or 24.1): "; read -r tgt
  if [[ -z "$tgt" ]]; then echo "No version entered; aborting."; return 0; fi

  _confirm "Proceed with rolling MAJOR upgrade on ${role} → ${tgt}? [y/N]: " || return 0
  _rolling_replace_image_for_role "$role" "$tgt" "$runtime"
  _printc "$YELLOW_BOLD" "All nodes now on ${tgt}. If auto-finalization is OFF, you must finalize."
  echo -n "Finalize now? [y/N]: "; read -r fin
  case "$fin" in y|Y|yes|YES) _finalize_major_upgrade_role "$role" "$runtime" ;; *) _printc "$YELLOW_BOLD" "Skipping finalization. Remember: rollback is only possible before finalization." ;; esac
}

finalize_upgrade_interactive() {
  heading "Finalize Major Upgrade" "Irreversible step — disables rollback"
  local runtime="${1:-$(_default_runtime)}"
  echo -n "Which side? [A=CA, B=CB] (default A): "; read -r side; side="${side:-A}"
  local role; role="$(_canon_cluster "$side" 2>/dev/null || echo "$side")"
  _confirm "Finalize any pending major-version upgrade on ${role}? [y/N]: " || return 0
  _finalize_major_upgrade_role "$role" "$runtime"
}

rollback_interactive() {
  heading "Rollback (before finalization)" "Re-run the rolling change with previous image"
  local runtime="${1:-$(_default_runtime)}"
  echo -n "Which side? [A=CA, B=CB] (default A): "; read -r side; side="${side:-A}"
  local role; role="$(_canon_cluster "$side" 2>/dev/null || echo "$side")"
  _printc "$YELLOW_BOLD" "NOTE: Rollback is ONLY POSSIBLE before finalization. After finalization, rollback to the previous MAJOR is not supported."
  echo -n "Target version to roll back TO (e.g., 24.1.17): "; read -r tgt
  if [[ -z "$tgt" ]]; then echo "No version entered; aborting."; return 0; fi
  _confirm "Proceed with rolling rollback on ${role} → ${tgt}? [y/N]: " || return 0
  _rolling_replace_image_for_role "$role" "$tgt" "$runtime"
}
...
while true; do
    ...
    echo "15) DB Console"
    echo "16) Settings"
    echo "17) Rolling PATCH change (upgrade/downgrade)"
    echo "18) Rolling MAJOR upgrade"
    echo "19) Finalize MAJOR upgrade"
    echo "20) Roll back to previous version"
    echo "q)  Quit"
    ...
    case "$choice" in
      ...
      12) check_row_counts ;;
      17) _confirm "Run rolling PATCH change now? [y/N]: " && upgrade_patch_interactive "$(_default_runtime)" ;;
      18) _confirm "Run rolling MAJOR upgrade now? [y/N]: " && upgrade_major_interactive "$(_default_runtime)" ;;
      19) _confirm "Finalize major upgrade now? [y/N]: " && finalize_upgrade_interactive "$(_default_runtime)" ;;
      20) _confirm "Run rollback (pre-finalization) now? [y/N]: " && rollback_interactive "$(_default_runtime)" ;;
